...
function* fetchPopularPlaces({ data, success, failure }) {
  try {
    const response = yield getRequest({
      API: `${api.URL.POPULAR_PLACES}?limit=6&sortOrder=-1`,
    });
...
function* fetchVehicleList({ data, success, failure }) {
  try {
    ...
    let formattedParams = '';
    keys.map((item, index) => {
      formattedParams = `${formattedParams}${index ? '&' : ''}${keys[index]}=${
    ...
    const response = yield getRequest({
      API: `${api.URL.VEHICLE_LISTING}?${formattedParams}`,
    });
    if (response.status === STATUS_CODE.unAuthorized) {
      yield put(setAuthorization(null));
      yield put(saveDriverData(null));
...
function* fetchTranmissionTypes({
  data,
  success = () => { },
  failure = () => { },
}) {
...
function* fetchVehicleCompleteDetails({
  id,
  success = () => { },
  failure = () => { },
}) {
  try {
    yield put(startLoader());
    let url = `${api.URL.VEHICLE_DETAILS}/${id.trim()}`;
    url = url.replace(/\u200B/g, '');
    const response = yield getRequest({
      API: url,
    });
